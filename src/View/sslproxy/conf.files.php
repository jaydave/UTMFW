<?php
/*
 * Copyright (C) 2004-2026 Soner Tari
 *
...
if (filter_has_var(INPUT_POST, 'reload')) {
	...
	$ruleSet= new RuleSet();
	if ($ruleSet->load('/etc/sslproxy/sslproxy.conf', NULL, $force)) {
		$View->RuleSet= $ruleSet;
		PrintHelpWindow(_NOTICE('Main sslproxy rules reloaded') . ': ' . $View->RuleSet->filename);
	} else {
		PrintHelpWindow('<br>' . _NOTICE('Failed loading main sslproxy rules'), NULL, 'ERROR');
	}
...
if (filter_has_var(INPUT_POST, 'load')) {
	...
	$filepath= "$SSLPROXY_CONFIG_PATH/$loadfile";
...
if (filter_has_var(INPUT_POST, 'save')) {
	...
	$filepath= "$SSLPROXY_CONFIG_PATH/$savefile";
...
if (filter_has_var(INPUT_POST, 'remove')) {
	...
	$filepath= "$SSLPROXY_CONFIG_PATH/$deleteFile";
...
if (filter_has_var(INPUT_POST, 'download')) {
	...
	if ($View->Controller($Output, 'GenerateRules', json_encode($View->RuleSet->rules), 0, $force) || $force) {
		...
		PrintHelpWindow('<br>' . _NOTICE('Failed downloading, cannot generate sslproxy rules'), NULL, 'ERROR');
	}
}
	...
	<tr>
		<td>
			...
			<form action="<?php echo filter_input(INPUT_SERVER, 'PHP_SELF'); ?>" method="post">
				...
				<label for="reload"><?php echo _CONTROL('Reload main ruleset') . ': /etc/sslproxy/sslproxy.conf' ?></label>
				<br />
			...
			<h2><?php echo _TITLE('Download CA certificate') ?></h2>
			<br />
			<form action="<?php echo filter_input(INPUT_SERVER, 'PHP_SELF'); ?>" method="post">
				<input type="submit" id="Download" name="Download" value="<?php echo _CONTROL('Download') ?>"/>
				<label for="Download"><?php echo _CONTROL('Download CA certificate to install on client web browsers, mail programs, or devices.') ?></label>
