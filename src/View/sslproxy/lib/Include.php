<?php
/*
 * Copyright (C) 2004-2026 Soner Tari
 *
 * This file is part of UTMFW.
 *
 * PFRE is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * PFRE is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with PFRE.  If not, see <http://www.gnu.org/licenses/>.
 */

namespace SSLproxy;

class _Include extends Rule
{
	function display($ruleNumber, $count)
	{
		$this->dispHead($ruleNumber, $count);
		$this->dispInclude();
		$this->dispTail($ruleNumber);
	}
	
	function dispInclude()
	{
		?>
		<td class="include">
			<?php echo 'Include' ?>
		</td>
		<td title="<?php echo _TITLE('File') ?>" colspan="3">
			<?php
			if (isset($this->rule['file'])) {
				echo $this->rule['file'];
			}
			?>
		</td>
		<?php
	}

	function input()
	{
		$this->inputKey('file');

		$this->inputKey('comment');
		$this->inputDelEmpty();
	}

	function edit($ruleNumber, $modified, $testResult, $generateResult, $action)
	{
		$this->editIndex= 0;
		$this->ruleNumber= $ruleNumber;

		$this->editHead($modified, $testResult, $generateResult, $action, 'sslproxy.conf');

		$this->editInclude();

		$this->editComment();
		$this->editTail();
	}

	function editInclude()
	{
		global $View, $SSLPROXY_CONFIG_PATH;

		$View->Controller($ruleFiles, 'GetRuleFiles');
		?>
		<tr class="<?php echo ($this->editIndex++ % 2 ? 'evenline' : 'oddline'); ?>">
			<td class="title">
				<?php echo _TITLE('File').':' ?>
			</td>
			<td>
				<select id="file" name="file">
					<option value=""></option>
					<?php
					foreach ($ruleFiles as $file) {
						$file= "$SSLPROXY_CONFIG_PATH/$file";
						?>
						<option value="<?php echo $file ?>" label="<?php echo $file ?>" <?php echo (isset($this->rule['file']) && $this->rule['file'] == $file ? 'selected' : ''); ?>><?php echo $file ?></option>
						<?php
					}
					?>
				</select>

			</td>
			<td class="none">
				<?php PrintHelpBox(_HELPBOX("Only files under the $SSLPROXY_CONFIG_PATH folder can be included here.")) ?>
			</td>
		</tr>
		<?php
	}
}
?>
