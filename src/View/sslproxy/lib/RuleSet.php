 ...
 * 
 * The View does not know how to parse or generate rules. It only maintains a list of rules
 ...
 * \li Enables user to configure settings
 */
namespace SSLproxy;
...
class RuleSet
{
	...
	function load($filename, $tmp= 0, $force= 0, $tmpFilename= '')
	{
		...
		$retval= TRUE;
		if ($filename == '/etc/sslproxy/sslproxy.conf') {
	...
	function test($ruleNumber, $ruleObj)
	{
		...
		// If we slice upto $ruleNumber only, sslproxy complains about no proxyspec, if the first proxyspec is below $ruleNumber
		...
		$rulesArray= json_decode(json_encode($this), TRUE)['rules'];
		$rulesArray[$ruleNumber]= json_decode(json_encode($ruleObj), TRUE);
	...
	function cancel()
	{
		if (filter_has_var(INPUT_POST, 'cancel') && (filter_input(INPUT_POST, 'cancel') == _CONTROL('Cancel'))) {
			...
			header('Location: /sslproxy/conf.php');
			exit;
		}
	}
