...
class Rule
{
	 ...
	 * @param string $help Anchor in the html file for the man page.
	...
	function editHead($modified, $testResult, $generateResult, $action, $man= 'sslproxy')
	{
		...
		?>
		<h2><?php echo $editHeader . ($modified ? ' (' . _TITLE('modified') . ')' : '') ?><?php $this->editHelp(ltrim($this->cat, '_'), $man) ?></h2>
	...
	/**
	 * Prints help image with a link to an anchor in the man page.
	 * 
	 * sslproxy and sslproxy.conf man pages are html files generated from the actual man pages.
	 * The anchors are added manually to match with the labels given here.
	 *
	 * @param string $label Anchor in the man page.
	 */
	function editHelp($label, $man= 'sslproxy')
	{
		global $IMG_PATH;
		?>
		<a target="<?php echo $label ?>" href="/sslproxy/<?php echo $man ?>.html#<?php echo $label ?>">
